...
func KeyAuthWithConfig(config KeyAuthConfig) echo.MiddlewareFunc {
	...
	case "all":
		extractor = keyFromAll(parts[1], config.AuthScheme)
	}
...
func keyFromAll(param string, authScheme string) keyExtractor {
	return func(c echo.Context) (string, error) {
		key := c.QueryParam(param)
		if key != "" {
			return key, nil
		...
		key = c.Request().Header.Get(param)
		if key != "" {
			return key, nil
		}

		if param == echo.HeaderAuthorization {
			l := len(authScheme)
			if len(key) > l+1 && key[:l] == authScheme {
				return key[l+1:], nil
			}
			return "", errors.New("invalid key in the request header")
		}

		return "", errors.New("missing key in header, form or query")
	}
}
